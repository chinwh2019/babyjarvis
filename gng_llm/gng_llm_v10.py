...
if __name__ == "__main__":
    ...
    # streamlit app
    # st.title('Diary Chatbot')
    
    # user = "user1"  # here you can make a login system to get the user name or id
    # user_input = st.text_input("What's on your mind today?", "")

    # if st.button('Talk'):
    #     if user_input:
    #         response = feedback_manager.generate_responses([user_input], user, chat_bot, model_manager)
    #         st.write(response)
            
    #         # Save the diary entry and the response to a local file (JSON format)
    #         with open(f"{user}_diary.json", "a") as f:
    #             json.dump({"entry": user_input, "response": response}, f)
    #             f.write("\n")  # newline for each diary entry

    #         # Provide a link to download the diary
    #         st.markdown(f'[Download your diary]({user}_diary.json)')
    #     else:
    #         st.write("Please input something first.")
