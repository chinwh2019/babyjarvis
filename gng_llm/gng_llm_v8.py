...
class ChatBot:
    ...
    # def generate_system_message(self, mood):
    #     if len(self.memory_manager.nodes) >= 1:
    #         past_messages = [msg['content'] for node in self.memory_manager.nodes[:-1] for msg in node['messages'] if msg['role'] == 'user'][-5:]
    ...
    def generate_messages(self, message, system_message):
        ...
        if similar_node is not None:
            ...
            messages.extend([{"role": "user", "content": msg["content"]} for msg in past_user_messages])
