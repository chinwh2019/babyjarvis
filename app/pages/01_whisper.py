    ...
    # -----------
        ...
        with st.expander("📝 &nbsp; Metadata"):
            ...
            st.markdown(
                ...
                <i>Transcript path</i>: `{Path(media["filepath"]).parent / "transcript"}[(.srt, .vtt, .json, .txt, 
                .tsv)]`<br/> """,
                unsafe_allow_html=True,
            )
        ...
        st.info(
            """Clicking on a segment will move start position to the segment (only audio player will continue playing 
            while video will pause)"""
        )
